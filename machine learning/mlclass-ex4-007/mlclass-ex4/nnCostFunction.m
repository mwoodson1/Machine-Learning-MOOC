...
%Need to map 1:9 to a binary vector
...
tmp = -y_matrix.*log(h) - (1-y_matrix).*log(1-h);
tmpsum = sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2));
J = (1/m)*sum(sum(tmp)) + lambda/(2*m)*tmpsum;
